...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Project Markdown agents | Scan `.snow/agents/**/*.md` (and optional `~/.snow/agents/**/*.md`); schedulable without editing `sub-agents.json`             |
| Merge priority          | `project .snow/agents` > `global ~/.snow/agents` > `~/.snow/sub-agents.json` > builtin                                         |
| `ROLE-*.md`             | Does **not** register a new `agentId` (role overlay only)                                                                      |
| Unknown-tool warn       | Unregistered frontmatter `tools` names are **warned**; declaration is not silently dropped; runtime still filters by allowlist |
| `beforeSubAgentStart`   | Hook can inject/rewrite the prompt before spawn (see [Hooks Configuration](./07.Hooks%20Configuration.md))                     |
...
- Plan Agent - A planning agent for developing comprehensive coding plans and guidance for the main workflow (persists plans via `plan-manage create/write_body`).
...
2. **Sub-Agent Selection**: Select an appropriate sub-agent based on the task type:
   ...
   - **Plan Agent**: Breaking down complex features, major refactoring planning; persist plans via `plan-manage` (`create` / `write_body`), not `filesystem-create` under `.snow/plan/**`
...
## Project Markdown Sub-Agents

In addition to the configuration UI and `~/.snow/sub-agents.json`, you can define sub-agents as Markdown files committed with the repo. They become discoverable and schedulable without editing `sub-agents.json`.

### Scan paths

- Project: `.snow/agents/**/*.md`
- Global (optional): `~/.snow/agents/**/*.md`
...
- `tools`: tool list (array or single string)
- `role`: optional frontmatter; body is used as role when omitted

Notes:

- Invalid markdown is skipped with a warning (no crash)
- `ROLE-*.md` under project root / `~/.snow` does **not** register a new `agentId` (role overlay only)
- Once discovered, agents are schedulable via `#agentId` / the picker
- Unknown tools in frontmatter are **warned** (declaration is not silently dropped; runtime still filters by allowlist)
...
## Sub-Agent Live Display (Live Slot)

While sub-agents run, Snow can show progress in the dynamic area as **agent containers**, instead of flooding static history with intermediate success tool cards.

### Default look (`slots`)

```text
  ◈ Explore Agent  (6s · Running)
  ◈ General Purpose Agent (5s · Running)
    └─ current tool/message (next overwrites previous)
    └─ +N more tools running…
```

- One container per sub-agent **instance** (parallel same-type agents stay distinct)
- Focus body **overwrites** the previous line
- Finished agents may keep a residual `Done` card until the main turn ends
- History usually keeps only the final `✓ subagent-agent_*` row

### Switching modes

Recommended unified entry:

- `/display` — open the display panel, select “Sub-agent display”, Enter to cycle
- `/display subagent [slots|multi|compact|hidden|status]`
- Compat: `/subagent-display …` and `snow cmd subagent-display …` / `snow cmd display subagent …`

| Arg       | Meaning                                               |
| --------- | ----------------------------------------------------- |
| `slots`   | **Default**. Agent container + single focus overwrite |
| `multi`   | Agent container + recent multi-line history           |
| `compact` | Header / elapsed only                                 |
| `hidden`  | Disable live panel; legacy tool cards                 |
| `status`  | Query only (no switch)                                |

The `/subagent-display` args picker shows 5 rows: 4 display schemes + `status` query. Labels include a short explanation for each item.

Persisted at: `~/.snow/theme.json` → `subAgentDisplayMode`.  
Full reference: [09.2 Mode Switching Commands · /display](./09.2.Mode%20Switching%20Commands.md#display-recommended-unified-entry) and [/subagent-display](./09.2.Mode%20Switching%20Commands.md#subagent-display).
