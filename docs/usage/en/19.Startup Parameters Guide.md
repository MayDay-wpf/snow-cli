...
#### 3. Headless Plan Resume (preapproved only)

Interactive plan approval is not available in headless. You can only **resume** a plan that is already `approved` or `executing`:

```bash
# Resume a specific plan file (rebinds session + owner lock when possible)
snow --ask "Continue the plan" --plan-file .snow/plan/2026-07-25/my-task.md <sessionId>

# Opt-in restore: only if this session already owns an executing plan
snow --ask "Continue" --plan <sessionId>

# --yolo-p with --ask uses the same restore path when an executing plan exists
snow --ask "Continue" --yolo-p <sessionId>
```

If resume fails (draft plan, lock conflict, no executing plan), headless stays with `planMode=false`.
