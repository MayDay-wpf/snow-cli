...
> **Related: tool category icons `toolIcons`**  
> Controls category emojis (🔍/💻/📖) next to tool names; independent of `toolDisplayMode`.  
> Config: `~/.snow/theme.json` → `toolIcons` (default `true`).  
> Commands: `snow cmd theme set toolIcons=on|off`, or `snow cmd theme set toolIcons=websearch-search:🔎`.  
