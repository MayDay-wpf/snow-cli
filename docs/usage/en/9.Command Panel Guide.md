...
Toggle vulnerability scanning mode.

- **Function**: Turn on/off security vulnerability detection mode, focusing on code security analysis
- **Use Cases**: Conducting security audits or code security checks
- **Status**: Status saved in localStorage
...
- **Function**: Review uncommitted changes in Git repository
- **Features**: 
  - Automatically detect Git repository
  - Get git diff
  - AI analyzes code quality, potential bugs, security issues
  - Provide optimization suggestions
- **Additional Message**: Can use `/review [your note]` to add review focus
- **Examples**: 
  - `/review` - Review all changes
  - `/review [focus on performance issues]` - Review with specific attention
...
- **Function**: Create ROLE.md file in project root directory to define AI's role and behavior
- **Use Cases**: Need to customize AI behavior and output style for specific project
- **Example**: Type `/role` to create file, then edit to define role
...
- **Use Cases**: After codebase update or when index is corrupted
- **Example**: Type `/reindex` to start rebuilding index
...
- [Sensitive Command Configuration](./6.Sensitive Command Configuration.md) - Configure dangerous operations that need confirmation
- [Hooks Configuration](./7.Hooks Configuration.md) - Configure automation before/after command execution
- [Codebase Settings](./4.Codebase Settings.md) - Configure codebase indexing feature (required for `/reindex`)
