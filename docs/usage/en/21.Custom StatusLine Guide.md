...
- `system.privacy`: privacy filter configuration snapshot, including `configured`, `enabled`, `mode`, `apiUrlConfigured`, `model`, and `toolResultTools`. This state is exposed only for plugins; Snow CLI itself does not render a default Privacy item in StatusLine.
...
| `builtin.privacy`            | No default rendering; plugin-only         | Readable whether privacy is configured |
