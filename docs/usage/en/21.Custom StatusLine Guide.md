...
- `system.speedometer`: real-time speedometer state, including `enabled` (whether enabled), `tps` (current tokens/s), `peakTps` (peak tokens/s). `enabled` is `true` only when the `/speedometer` command is active.
...
| `builtin.speedometer`        | `⏱ {tps} tok/s · peak {peakTps}`          | Speedometer is enabled (via `/speedometer` command)                                              |
