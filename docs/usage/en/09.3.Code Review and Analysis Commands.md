...
- **Runtime effect**: Project `AGENTS.md` is part of the built-in context inject chain. When `contextInject.enabled` is true (default off), Snow prepends global + project `AGENTS.md` (root → cwd) to **model-bound user messages**; the chat bubble still shows your typed text. Enable with `/agents-inject on` (or session-command `agents-inject`). See [AGENTS.md Context Inject](./30.AGENTS%20Context%20Inject.md).
...
- **Inject path**: ROLE stays on the **system prompt** path (not the AGENTS user-message prepend). Global + project active ROLE can both load; identical bodies are content-deduped. This is separate from [AGENTS.md Context Inject](./30.AGENTS%20Context%20Inject.md).
