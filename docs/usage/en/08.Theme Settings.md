...
- **toolIcons**: Tool category icons (e.g. 🔍 / 💻 / 📖) on/off and overrides
  - `true` (default): show category icons next to tool names
  - `false`: hide category icons (status glyphs ⚡/✅/❌ remain)
  - Object form for per-tool overrides:
    ```json
    {
    	"enabled": true,
    	"tools": {
    ...
    ```
  - Independent of `toolDisplayMode` (compact + icons on is valid)

### Tool icons (`toolIcons`) commands

```bash
# Status (includes toolIcons)
snow cmd theme status

# Disable / enable category icons
snow cmd theme set toolIcons=off
snow cmd theme set toolIcons=on

# Override one tool icon
snow cmd theme set toolIcons=websearch-search:🔎

# Clear an override (restore built-in)
snow cmd theme set toolIcons=websearch-search:
```

You can also edit the `toolIcons` field in `~/.snow/theme.json` directly.
