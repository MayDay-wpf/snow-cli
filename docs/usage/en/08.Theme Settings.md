...
## Display density (tool / think / sub-agent)

Theme Settings exposes three display controls at the top of the menu (same config as slash commands):

| Menu row | Config field | Cycle | Equivalent command |
| -------- | ------------ | ----- | ------------------ |
| Tool display | `toolDisplayMode` | `full` → `compact` → `hidden` | `/display tool …` / `/tool-display` |
| Think display | `thinkDisplayMode` | `compact` → `full` | `/display think …` / `/think-display` |
| Sub-agent display | `subAgentDisplayMode` | `slots` → `multi` → `compact` → `hidden` | `/display subagent …` / `/subagent-display` |

- In Theme Settings: select a row and press **Enter** to cycle; values write to `theme.json` and hot-sync the current session
- Faster in chat: run `/display` to open the lightweight display panel
- Details: [09.2 Mode Switching Commands · /display](./09.2.Mode%20Switching%20Commands.md#display-recommended-unified-entry)
...
> `toolIcons: true` still works: category icons on by default, status prefixes use compact built-ins (`✓/·/✗`).
...
- **toolDisplayMode**: Tool-call display density (exactly three modes; no fourth tier)
  ...
  - Also via `/display tool …`, `/tool-display`, or `snow cmd display tool …`

- **thinkDisplayMode**: Thinking content display mode

  - `compact` (default) / `full`
  - Also via `/display think …`, `/think-display`, or `snow cmd display think …`

- **subAgentDisplayMode**: Sub-agent live panel presentation

  - `slots` (default): agent container + current focus overwrite
  - `multi`: agent container + recent multi-line history
  - `compact`: agent header / elapsed only
  - `hidden`: disable live panel (legacy tool cards)
  - Also via `/display subagent …`, `/subagent-display`, or `snow cmd display subagent …`
  - Chat shortcut `/display` opens the unified interactive panel; see [09.2 Mode Switching Commands · /display](./09.2.Mode%20Switching%20Commands.md#display-recommended-unified-entry)

- **toolIcons**: Chat tool-title **category icons** and **status prefixes** (not the bottom StatusLine)
  - Title shape: `[status] [category icon] [display name or technical id]` — empty parts omitted
  - `true` (default): category icons on; status prefixes use compact defaults `✓ / · / ✗ / ! / …`
  - `false`: hide **category** icons; status prefixes stay on by default (turn off via object `status`)
  - Object form:
    ```json
    {
    	"enabled": true,
    ...
    ```
  - `status: false` or `status.enabled: false`: hide status prefixes (e.g. only `💻 Terminal`)
  - Independent of `toolDisplayMode` and `toolDisplayNames`

### Tool icons (`toolIcons`) commands

**TUI slash (recommended):**

```bash
/tool-icons                      # status (category + status prefixes)
/tool-icons status               # same
/tool-icons on|off               # category icons
/tool-icons status on|off        # status prefixes
/tool-icons status:success:✓     # set success glyph (pending/error/warning/running too)
/tool-icons status:success:      # restore default for that key
/tool-icons success:✅           # shorthand (reserved status key)
/tool-icons websearch-search:🔎  # override one tool category icon
/tool-icons websearch-search:    # clear category override
```

**session-command:**

```bash
# Status (includes toolIcons / toolStatusIcons / toolDisplayNames)
snow cmd theme status

# Category icons
snow cmd theme set toolIcons=off
snow cmd theme set toolIcons=on

# Status prefixes
snow cmd theme set toolIcons=status:off
snow cmd theme set toolIcons=status:on
snow cmd theme set toolIcons=status:success:✓
snow cmd theme set toolIcons=status:success:

# Override one tool category icon
snow cmd theme set toolIcons=websearch-search:🔎

# Clear one category override (restore built-in)
snow cmd theme set toolIcons=websearch-search:
```

You can also edit the `toolIcons` field in `~/.snow/theme.json` (prefer slash / session-command over whole-file overwrite).

### Tool display names (`toolDisplayNames`)

Chat tool titles use the technical tool id by default (e.g. `websearch-search`). **There are no built-in localized names** — only your overrides change the label.

> **Prefer the official path** (agents too): `/tool-names` or `snow cmd theme set toolDisplayNames=…`.  
> **Avoid** whole-file `filesystem-create/edit` of `theme.json` — bypasses the theme API and can wipe `customColors` / other fields.

**TUI slash** (`/tool-name` is an alias):
...
```bash
snow cmd theme set toolDisplayNames=websearch-search:Web Search
snow cmd theme set toolDisplayNames=websearch-search:
...
MCP / skills: without an override, the technical id is shown. Affects **new** tool titles only.
...
4. Theme file watches hot-refresh colors/modes in a running TUI; after source rebuild, restart Snow once to load the new bundle

Prefer the configuration UI or `theme colors` / `theme set` session commands to avoid format errors.
