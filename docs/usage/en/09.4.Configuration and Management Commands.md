...
- **Function**: Create a loop task that periodically executes a specified prompt at a fixed interval. By default, loops are session-scoped and stop when the current Snow CLI session exits; daemon mode can keep a loop running in the background.
- **Syntax**:
...
- **Session vs daemon mode**:
  - Session-scoped loop: created with `/loop 5m <prompt>`, valid only in the current Snow CLI session, and stops when the session exits
  - Daemon loop: created with `/loop daemon 5m <prompt>` or `/loop --daemon 5m <prompt>`, starts a detached background process, and keeps running after the current Snow CLI session exits
