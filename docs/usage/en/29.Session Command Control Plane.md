...
snow cmd buddy set --hat=crown --eye=✦ --rarity=legendary --shiny=true --json
...
| `buddy.set`     | `snow cmd buddy set --hat=… --eye=… …`      | low_write | no      | Customize look/personality; `customize` is an alias |
...
| Command ID          | CLI form                                    | Risk      | Confirm | Notes              |
| ------------------- | ------------------------------------------- | --------- | ------- | ------------------ |
| `theme.status`      | `snow cmd theme status`                     | read      | no      | Bare `theme`       |
| `theme.set`         | `snow cmd theme set <name>`                 | low_write | no      |                    |
...
| `codebase`          | `snow cmd codebase [on\|off\|status]`        | medium_write | yes\*   | status is read            |
...
| `usage`             | `snow cmd usage`                             | read         | no      | Headless usage summary    |
...
| Edit theme / simple / tool-display files          | `theme status                                                      | set`, `simple`, `tool-display`, `think-display` |
...
| Read raw settings for agent automation            | `config snapshot` (non-secret fields only)                         |
...
- `reindex` requires codebase to be configured; otherwise `NOT_CONFIGURED` / `EXECUTION_FAILED`.
...
- Light pilot: plane `simple` / `tool-display` also emit `configEvents` so headless changes notify TUI subscribers.
- Same-process Agentic/SSE session-command writes to plan/yolo/team/theme/display modes now emit `configEvents` so the live TUI status line updates immediately. External cross-process `snow cmd` does not hot-refresh an already-open TUI process.
...
- Contract tests in `source/test/session-command-plane.test.ts` cover:
  ...
  - same-process `configEvents` emission for plan/yolo/theme writes
