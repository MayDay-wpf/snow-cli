...
**Purpose**: Automatically optimize user input prompts to improve AI understanding accuracy
...
**Recommendation**: Keep enabled to improve interaction quality
...
**Purpose**: Specialized model for text compression and summarization

**Configuration Method**: Same as Advanced Model

**Default**: Optional (if not filled, compression model will not be used)

**Description**:

- Compact model uses the same Base URL and API Key as the main model
- Recommend using small, fast models to improve compression efficiency
