...
After entering the task manager, you'll see tasks for the current project by
default. Press `G` to deliberately switch to the all-projects view; that view
labels each task with its project origin. Each task displays:
...
- `R` - Refresh task list
- `G` - Switch between the current-project and all-projects views
