...
Create a GitHub pull request (PR) via AI prompt.

- **Function**: Guide the AI to stage/commit changes, push the current branch, and create a PR using GitHub CLI (`gh`)
- **Prerequisites**: `git` and `gh` installed, and `gh auth login` completed
- **Why this exists**: Avoid repeating manual steps (branch → commit → push → create PR) and avoid PR body newline escaping issues
- **Newline safety**: The prompt enforces `gh pr create --body-file <path>` (do not use `--body \"...\\n...\"`)
- **Examples**:
  - `/git pr` - Create PR with auto-generated title/body guidance
  - `/git pr Add /git pr command` - Use the given text as the PR title hint
