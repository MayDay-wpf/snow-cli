...
主题设置页顶部提供三项显示开关（与 slash 同配置）：

| 菜单项     | 配置字段              | 循环值                                   | 等价命令                                    |
| ---------- | --------------------- | ---------------------------------------- | ------------------------------------------- |
| 工具显示   | `toolDisplayMode`     | `full` → `compact` → `hidden`            | `/display tool …` / `/tool-display`         |
| 思考显示   | `thinkDisplayMode`    | `compact` → `full`                       | `/display think …` / `/think-display`       |
| 子代理显示 | `subAgentDisplayMode` | `slots` → `multi` → `compact` → `hidden` | `/display subagent …` / `/subagent-display` |

- 在主题设置中：选中后按 **Enter** 循环切换，立即写入 `theme.json` 并热更新当前会话
- 聊天中更快入口：直接输入 `/display` 打开轻量显示面板
- 详情见 [09.2 模式切换指令 · /display](./09.2.模式切换指令.md#display推荐统一入口)
...
> `toolIcons: true` 仍可用：表示类型图标默认开、状态前缀用内置紧凑符号（`✓/·/✗`）。
...
- **subAgentDisplayMode**: 子代理实时面板显示方案

  - `slots`（默认）：agent 容器 + 当前焦点覆盖
  - `multi`：agent 容器 + 最近多行历史
  - `compact`：仅 agent 标题/耗时
  - `hidden`：关闭实时面板，回退旧工具卡片
  - 也可用 `/subagent-display` 或 `snow cmd subagent-display`
  - 详见 [09.2 模式切换指令](./09.2.模式切换指令.md#subagent-display)

- **toolIcons**: 聊天区工具标题的**类型图标**与**状态前缀**（与底部 StatusLine 无关）
  - 标题结构：`[状态前缀] [类型图标] [显示名或技术 ID]`，空段自动省略
  - `true`（默认）：类型图标开；状态前缀用紧凑默认 `✓ / · / ✗ / ! / …`
  - `false`：关闭**类型**图标；状态前缀仍默认开启（可用对象里的 `status` 单独关）
  - 对象形式：
    ```json
    {
    	"enabled": true,
    ...
    ```
  - `status: false` 或 `status.enabled: false`：隐藏状态前缀（例如只显示 `💻 终端命令`）
  - 与 `toolDisplayMode`、`toolDisplayNames` **独立**
...
```bash
/tool-icons                      # 查看状态（类型 + 状态前缀）
/tool-icons status               # 同上
/tool-icons on|off               # 启停类型图标
/tool-icons status on|off        # 启停状态前缀
/tool-icons status:success:✓     # 改成功符号（pending/error/warning/running 同理）
/tool-icons status:success:      # 恢复该状态默认符号
/tool-icons success:✅           # 简写（保留状态 key）
/tool-icons websearch-search:🔎  # 覆盖单个工具类型图标
/tool-icons websearch-search:    # 清除类型覆盖
```

**session-command：**

```bash
# 查看（含 toolIcons / toolStatusIcons / toolDisplayNames）
...
也可直接编辑 `~/.snow/theme.json` 的 `toolIcons` 字段（推荐仍走 slash / session-command，避免整文件覆盖）。

### 工具显示名（toolDisplayNames）

聊天区工具标题默认显示技术 ID（如 `websearch-search`）。**无内置中英文默认译名**；只有你配置了覆盖才会改显示。

> **推荐官方路径**（Agent / 自动化同样适用）：`/tool-names` 或 `snow cmd theme set toolDisplayNames=…`。  
> **不推荐**用 `filesystem-create/edit` 整文件覆盖 `theme.json`——会绕过 theme API，且容易冲掉 `customColors` 等其它字段。

**TUI 斜杠**（`/tool-name` 为同义别名）：
...
```bash
snow cmd theme set toolDisplayNames=websearch-search:网页搜索
...
外挂 MCP / skills：未配置则仍显示原 ID；需要中文时自行覆盖即可。仅影响**新**工具标题。
...
3. 颜色值必须是有效的颜色格式
4. 保存后会自动热刷新，无需重启 Snow CLI（TUI 会监听 `~/.snow/theme.json`）

**Agent / 自动化推荐写法（不要直接强写文件）：**

```bash
# 通过 session-command 写入自定义配色并立即热刷新
snow cmd theme colors "{\"background\":\"#0B0E1A\",\"text\":\"#E2E4F0\",\"menuSelected\":\"#A78BFA\"}"
# 或
snow cmd theme set custom
# 工具类型图标
snow cmd theme set toolIcons=on
snow cmd theme set toolIcons=terminal-execute:💻
```

建议优先使用配置界面或 `theme colors` / `theme set` 命令；直接覆盖 `theme.json` 也能热刷新，但缺少校验。
