...
- **作用**: 创建一个按固定间隔周期性执行指定 Prompt 的循环任务。默认是会话级循环，退出当前 Snow CLI 后停止；也可以使用 daemon 模式创建后台常驻循环。
- **语法格式**:
...
- **会话级与 daemon 模式区别**:
  - 会话级循环：通过 `/loop 5m <prompt>` 创建，只在当前 Snow CLI 会话内有效，退出后停止
  - daemon 循环：通过 `/loop daemon 5m <prompt>` 或 `/loop --daemon 5m <prompt>` 创建，会启动 detached 后台进程，退出当前 Snow CLI 后仍继续运行
