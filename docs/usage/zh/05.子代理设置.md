...
- Plan Agent——计划代理，用于为主流程制定全面的编码计划与指导（通过 `plan-manage create/write_body` 落盘）
...
2. **子代理选择**: 根据任务类型选择合适的子代理：
   ...
   - **Plan Agent**: 复杂功能拆解、重大重构规划；计划落盘使用 `plan-manage`（`create` / `write_body`），不要用 `filesystem-create` 写 `.snow/plan/**`
...
子代理运行时，Snow 可在动态区用 **agent 容器** 展示进度，而不是把中间工具成功卡刷进静态历史。
...
- 每个子代理**实例**一个容器（同一类型并行也不会串号）
- 容器内当前焦点**覆盖**上一条
- 子代理结束后可短暂保留 residual `Done`，主 turn 结束再清理
- 历史区通常只保留最终 `✓ subagent-agent_*`，不堆中间成功卡片

### 切换显示方案

推荐统一入口：

- `/display` — 打开显示面板，选「子代理显示」后 Enter 循环
- `/display subagent [slots|multi|compact|hidden|status]`
- 兼容：`/subagent-display …` 与 `snow cmd subagent-display …` / `snow cmd display subagent …`

| 参数      | 含义                                |
| --------- | ----------------------------------- |
| `slots`   | **默认**。agent 容器 + 单行焦点覆盖 |
| `multi`   | agent 容器 + 最近多行历史           |
| `compact` | 仅标题/耗时                         |
| `hidden`  | 关闭实时面板，回退旧工具卡片        |
| `status`  | 只查询，不切换                      |

`/subagent-display` 补全面板会看到 5 行：4 种显示方案 + `status` 查询。命令说明里也会简要标注每项含义。

持久化字段：`~/.snow/theme.json` → `subAgentDisplayMode`。  
完整说明见 [09.2 模式切换指令 · /display](./09.2.模式切换指令.md#display推荐统一入口) 与 [ /subagent-display](./09.2.模式切换指令.md#subagent-display)。
