...
- `config snapshot` 包含非密钥字段：modes、theme、`speedometerEnabled`、`hybridCompressEnabled`、`autoFormatEnabled`、`subAgentMaxSpawnDepth`、`fileListDisplayMode`、`language`、`showThinking`、`privacy:{enabled,mode}`、`codebaseFlags` 等。
...
- 已覆盖热同步（同进程）：`simple`、`tool-display`、`think-display`、`show-thinking`、`image-compress`、`hybrid-compress`、`speedometer`、`auto-format`、`yolo`、`plan`、`tool-search`、`vulnerability-hunting`、`team`、`ultra-todo`、`theme`/`diffOpacity`、`language`、`file-list-display`、`privacy`、`telemetry`、`codebase`（含 flags）、`subagent-depth`。
