...
| `buddy.set`     | `snow cmd buddy set --hat=… --eye=… …`      | low_write | 否   | 自定义外观/性格；`customize` 同义 |
...
| 命令 ID             | CLI 形式                                    | 风险      | 确认 | 说明           |
| ------------------- | ------------------------------------------- | --------- | ---- | -------------- |
| `theme.status`      | `snow cmd theme status`                     | read      | 否   | 裸 `theme`     |
| `theme.set`         | `snow cmd theme set <name>`                 | low_write | 否   |                |
...
| `codebase`          | `snow cmd codebase [on\|off\|status]`        | medium_write | 是\* | status 只读       |
...
| `usage`             | `snow cmd usage`                             | read         | 否   | 无头用量摘要      |
...
| `config.snapshot`      | `snow cmd config snapshot`      | read | 否   | 裸 `config`；无密钥字段         |
...
| 手改 theme / simple / tool-display            | `theme status                                                      | set`、`simple`、`tool-display`、`think-display` |
...
| Agent 读取原始 settings                       | `config snapshot`（仅非密钥字段）                                  |
...
- 轻量试点：plane 的 `simple` / `tool-display` 写入时也会 `configEvents` 通知 TUI 订阅者。
- 同进程内 Agentic/SSE 的 session-command 写入 plan/yolo/team/theme/display 等模式时会 emit `configEvents`，使当前 TUI 状态栏立即热更新。跨进程外部 `snow cmd` 不会热刷新已经打开的 TUI 进程。
...
- `source/test/session-command-plane.test.ts` 覆盖：
  ...
  - 同进程 plan/yolo/theme 写路径的 `configEvents` 发射
