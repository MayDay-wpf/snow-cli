...
- `system.speedometer`：实时测速仪状态，包含 `enabled`（是否启用）、`tps`（当前 token/s）、`peakTps`（峰值 token/s）。仅当 `/speedometer` 命令启用时 `enabled` 为 `true`。
...
| `builtin.speedometer`        | `⏱ {tps} tok/s · peak {peakTps}`        | 测速仪已启用（通过 `/speedometer` 命令开启）                                   |
