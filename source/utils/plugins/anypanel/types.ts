 ...
 * 若两者都存在，优先使用 `render()`。
 */
export interface AnyPanelPlugin {
	 ...
	 */
	render?(state: unknown, ctx: AnyPanelRenderContext): AnyPanelReactResult;
	 ...
	 */
	getRenderLines?(state: unknown): AnyPanelRenderResult;

	/**
	 * 查询当前面板状态。
	 * 返回 'done' 时 AnyPanelScreen 会关闭面板。
	 */
	getStatus(state: unknown): AnyPanelStatus;
