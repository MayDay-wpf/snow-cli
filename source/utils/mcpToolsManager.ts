 ...
 */
async function probeServiceTools(serviceName: string, server: MCPServer): Promise<InternalMCPTool[]> {
 ...
 */
async function connectAndGetTools(serviceName: string, server: MCPServer, timeoutMs: number = 10000): Promise<InternalMCPTool[]> {
      ...
      try {
        ...
        await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('HTTP connection timeout')), timeoutMs)
          )
        ]);
      } catch (httpError) {
        // Fallback to SSE
 ...
 */
async function executeOnExternalMCPService(serviceName: string, server: MCPServer, toolName: string, args: any): Promise<any> {
  let client: Client | null = null;
